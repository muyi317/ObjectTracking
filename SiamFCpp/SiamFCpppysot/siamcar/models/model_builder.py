...
from SiamFCpp.SiamFCpppysot.siamcar.models.loss_car import make_siamcar_loss_evaluator
from SiamFCpp.SiamFCpppysot.siamcar.models.backbone import get_backbone
from SiamFCpp.SiamFCpppysot.siamcar.models.head.car_head import CARHead
from SiamFCpp.SiamFCpppysot.siamcar.models.neck import get_neck
from SiamFCpp.SiamFCpppysot.siamcar.utils.location_grid import compute_locations
from SiamFCpp.SiamFCpppysot.siamcar.utils.xcorr import xcorr_depthwise
...
class ModelBuilder(nn.Module):
    ...
    def track(self, x):
        ...
        # elif cfg.BACKBONE.TYPE=='alexnet':
            ...
            # （1）单个分支相关操作 siamcar  - alexnet 
            #  features = self.xcorr_depthwise(xf,self.zf) #[1, 256, 31, 31]  [1,256, 7,7] --> [1, 256, 25, 25]
            
        # 重点FCOS
        cls, loc, cen = self.car_head(self.zf, xf) #[1, 256, 25, 25]
    ...
    def forward(self, data):   # data: template[B,C,TEMP_SIZE,TEMP_SIZE], search[B,C,SEARCH_SIZE,SEARCH_SIZE], label_cls[B,OUTPUT_SIZE,OUTPUT_SIZE], bbox[B,4]
        ...
        if len(data)>=4:
            ...
            cls, loc, cen = self.car_head(zf, xf) #[32, 256, 25, 25]--> cls[B,2,25,25], loc[B,4,25,25], cen[B,1,25,25]
