...
import os
log = Logger(os.path.join('D:\Projects\PycharmProject\ObjectTracking\SiamFCpp\SiamFCpppysot//bin\logs', 'log.txt'), title=None)
log.set_names(['Tracker Name   Accuracy   Robustness   Lost Number   EAO'])
    ...
    def show_result(self, result, eao_result=None, show_video_level=False, helight_threshold=0.5):
        ...
        # for tracker_name, ret in result.items():
            ...
            if eao_result is None:
                ...
                log.append([formatter.format(tracker_name, accuracy, robustness, lost_number, eao_result[tracker_name]['all'])])
        print(bar)
