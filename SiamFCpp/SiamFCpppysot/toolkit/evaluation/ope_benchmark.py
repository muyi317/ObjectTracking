...
class OPEBenchmark:
    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                if len(video.gt_traj[0]) == 8:
                    gt_traj4 = []
                    for i in video.gt_traj:
                        j = []
                        j.append(i[0])
                        j.append(i[1])
                        j.append(i[2] - i[0])
                ...
                # result_bb_0 = []
                # for i in tracker_traj:
                #     if len(i) == 1:
                #         result_bb_0.append([0, 0, 0, 0])
                #     else:
                #         result_bb_0.append(i)
                # tracker_traj = np.array(result_bb_0)
    ...
    def plot_success(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if hasattr(video, 'absent'):
                    gt_traj = gt_traj[video.absent == 1]
                    tracker_traj = tracker_traj[video.absent == 1]
                success_ret_[video.name] = success_error(gt_traj, tracker_traj, n_frame)
            success_ret[tracker_name] = success_ret_
        return success_ret